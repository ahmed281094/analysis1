...
const log = logger.child("analyses.controller");
...
export const createAnalyses = async (req: Request, res: Response) => {
	try {
		...
		if (cachedScore) {
			log.info("Serving performance score from Redis cache...");
		...
		if (!runnerResult?.report) {
			log.error("Lighthouse did not return a report");
			return res
				.status(500)
		...
		if (performanceScore !== null) {
			...
			log.info(`Cached performance score for ${url} -> ${performanceScore}`);
		}
		...
		log.error("Failed to analyze website:", err);
...
export const getAnalysesByWebsite = async (req: Request, res: Response) => {
	try {
		...
		if (cached) {
			...
			log.info(`Serving analyses for website ${websiteIdStr} from cache`);
			return res.status(200).json({
				fromCache: true,
				result: parsed,
			});
		...
		log.error("Failed to fetch analyses:", err);
